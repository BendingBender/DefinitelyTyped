// Type definitions for inquirer 3.2
...
interface Question {
    /**
     * Type of the prompt. Defaults: `input` - Possible values: `input`, `confirm`, `list`, `rawlist`, `expand`,
     * `checkbox`, `password`, `editor`
     * @defaults: 'input'
     */
    type?: 'input' | 'confirm' | 'list' | 'rawlist' | 'expand' | 'checkbox' | 'password' | 'editor';
    /**
     * The name to use when storing the answer in the answers hash. If the name contains periods, it will define
     * a path in the answers hash.
     */
    name?: string;
    /**
    ...
    /**
     * Choices array or a function returning a choices array. If defined as a function, the first parameter will be
     * the current inquirer session answers.
     * Array values can be simple `strings`, or `objects` containing a `name` (to display in list), a
     * `value` (to save in the answers hash) and a `short` (to display after selection) properties. The choices
     * array can also contain `Separator`.
     */
    choices?: ChoiceType[] | ((answers: Answers) => ChoiceType[]);

    /**
     *  Receive the user input and answers hash. Should return `true` if the value is valid, and an
...
declare namespace ui {
     ...
     */
    class Prompt extends BaseUI<Prompts> {
